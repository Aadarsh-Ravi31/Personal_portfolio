 ...
 *
 * The build unifies two cities, Chicago and Dallas, on Azure Data Factory,
 * Databricks and Snowflake, in a Medallion (Bronze → Silver → Gold)
 * architecture.
 *
 * Two shapes make the argument: a long feed (Chicago, every violation packed
 * into one delimited string) and a wide feed (Dallas, up to 25 numbered blocks
 * across dozens of columns). Nothing about reading one helps you read the
 * other, which is why the pipeline runs as two city-specific lanes that meet at
 * a shared violation grain rather than one generic path with a city flag.
 *
 * Figures here describe the design and come from profiling the source feeds.
 */
...
export const SCALE: Stat[] = [
  ...
  { value: "2", label: "Source shapes" },
  { value: "3", label: "Medallion layers" },
  { value: "25", label: "Dallas violation blocks" },
  { value: "5", label: "Gold entities" },
  { value: "4", label: "Star dimensions" },
];

/**
 * The whole problem in one table. Same domain, two incompatible shapes, which
 * is why one generic pipeline could not have worked.
 *
 * The severity row matters more than it looks. The two cities do not measure
 * danger the same way or even at the same level, so there is no honest way to
 * put them on a shared severity scale. The dashboards show them side by side
 * and say so rather than normalising them into a single invented number.
 */
export const CITIES = [
  {
    city: "Chicago",
  ...
  },
  {
    ...
    shape: "Wide",
    transform: "Unpivot",
    violations: "Up to 25 numbered blocks spread across dozens of columns",
    severity: "Assigns numeric point deductions",
    quirks: [
      "Coordinates nested inside a lat_long object",
      "Violation blocks thin out sharply after the first few",
      "Numbered blocks must be unpivoted to a long shape",
    ],
  },
];

/**
 * Profiling findings, measured against the source feeds before any
 * transformation was written.
 */
export const FINDINGS = [
  ...
  },
  {
    ...
    detail: "Pipe-separated free text, several violations per inspection",
...
export const STAGES: Stage[] = [
  ...
  },
  {
    ...
    output: [{ value: "3", label: "findings acted on" }],
  },
  {
    id: "transform",
    step: "03",
    ...
    decision: {
      ...
      because: [
        "Unpivoting and text-parsing share no logic",
        "A generic path would branch on city at every step anyway",
        "City-specific code is readable; a branching monolith is not",
      ],
      cost: "A third city means writing a third transformation, not adding a config entry.",
    },
    output: [
  ...
  },
  {
    ...
    decision: {
      ...
      because: [
        ...
        "Violation grain is the only one both cities can reach",
      ],
